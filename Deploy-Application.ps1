...
Try {
    ...
    [String]$appName = 'Azure Batch Explorer'
    [String]$appVersion = ''
    [String]$appArch = ''
    [String]$appLang = 'DE'
    ...
    If ($deploymentType -ine 'Uninstall' -and $deploymentType -ine 'Repair') {
		...
		$$OtherPath = Get-ChildItem -Path "$dirFiles" -Include BatchExplorer.Setup*.exe -File -Recurse -ErrorAction SilentlyContinue

        If($$OtherPath.Exists) {
			## Install Azure Batch Explorer
			Execute-Process -Path "$$OtherPath" -Parameters "/allusers /S" -WindowStyle Hidden
        }
    ...
    ElseIf ($deploymentType -ieq 'Uninstall') {
        ...
        Show-InstallationWelcome -CloseApps 'BatchExplorer' #-CloseAppsCountdown 60
        ...
        $AppList = Get-InstalledApplication -Name 'BatchExplorer'
        
        ForEach ($App in $AppList) {
			If($App.UninstallString) {
				$SourceCache = $App.UninstallString -replace '"', '' -replace '/allusers', ''
				
				If(Test-Path -Path $SourceCache) {
					Execute-Process -Path $SourceCache -Parameters '/allusers /S'
				}
			}
        }
